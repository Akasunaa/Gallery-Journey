...
class Inventory {
public:
    explicit Inventory(std::map<std::string, std::unique_ptr<Material>> materials, std::map<std::string, std::unique_ptr<Equipment>> equipments);
    void init_inventory(pugi::xml_node node);
    void add_object_type(const Equipment & equip_type);
    void add_object_type(const Material & mat_type);
